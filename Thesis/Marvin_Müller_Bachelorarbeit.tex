...
\\
Folgendes Beispiel zeigt einen Testfall, welcher die Anforderungen aus der in Kapitel \ref{sec:testdaten} gezeigten Anforderungsspezifikation testen soll, welcher nicht die Testfallstruktur erfüllt und somit nicht weiter berücksichtigt wird, da er unbrauchbar ist.
\begin{figure}[H]
    \centering
...
Die Testschritte eines Testfalls müssen stimmig sein und den Prozess in der richtigen Reihenfolge wiedergeben, wie er auf der Webseite \textcite{OpenCart2025} abläuft. So kann ein Nutzer sich nicht von einer Anwendung abmelden, wenn er sich nicht vorher angemeldet hat. Ein Testfall muss somit als ersten Testschritt Anmeldung besitzen, bevor dieser den Testschritt Abmeldung hat. Deshalb ist dieser Testfall unbrauchbar, da dieser nicht die Logik der Anforderungsspezifikation erfüllt. Um die Logik und die Korrektheit der generierten Testfälle bestimmen zu können, werden die manuell erstellten Testfälle von \textcite{Pavankumar21github2022} als Benchmark genutzt. Diese werden pro Testszenario mit den generierten Testfällen verglichen.\\
\\
...
\begin{itemize}
    \item \text{True Positive (TP)} Anzahl faktisch korrekten Testschritte pro Testfall,
    \item \text{False Positive (FP)} Anzahl faktisch nicht korrekten Testschritte pro Testfall, d.h. Testschritte welche sich nicht in der Anforderungsspezifikation oder den Screenshots des Testszenario befinden,
...
\noindent Anschließend wird der Mittelwert berechnet, um die Gesamtrate aller Testfälle zu ermitteln. Von einem Testfall wird erwartet, dass er sich in der 1. Kategorie befindet.\\
\\
