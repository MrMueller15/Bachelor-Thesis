...
\\
Die Autoren \textcite{Li2024} erforschen das Thema Halluzinationen im Bereich der LLM. Das Ziel ist es, Halluzinationen zu erkennen und die Anzahl der Halluzinationen zu reduzieren. Dabei kommen \textcite{Li2024} zu dem Schluss, dass die Anzahl der Halluzinationen durch gezieltes Training und RAG, verringert werden, jedoch nicht vollständig eliminert werden kann. Da in dieser Arbeit, dass LLM ChatGPT-4 von \textcite{OpenAI2025} verwendet wird, ist es wichtig, dass Phänomen der Halluzination zu untersuchen. Die von \textcite{Li2024} aufgestellte Methodik zur Messung der Halluzinationsrate (HR), wird in dieser Arbeit in Kapitel \ref{sec:ausschlusskriterien} verwendet.
(\cite{Li2024})
\newpage
...
\noindent generierten Testfälle können nur bewertet werden, wenn es manuelle Testfälle gibt, die die gleichen Funktionen und Anforderungen abdecken. Diese sind ebenfalls in den GitHub Projekten zu finden. Folgende Grafik zeigt einen Testfall die von \textcite{Pavankumar21github2022} erstellt wurden ist und welcher die Erstellung eines Accounts in \textcite{OpenCart2025} testen.
\begin{figure}[H]
    \centering
...
\noindent Mit Hilfe dieser Anforderungsspezifikation und von Screenshots der jeweiligen Webseite, generiert ChatGPT-4 von \textcite{OpenAI2025} Testfälle. Wie dieser Vorgang genau abläuft wird in Kapitel \ref{sec:promptUndVerwendeteMethoden} erläutert. Diese generierten Testfälle werden in Kapitel \ref{sec:auswertungsmetriken} mit Hilfe der manuellen Testfälle auf ihre Richtigkeit überprüft und gemessen.
...
Im Bereich des Testens gibt es viele Metriken und Qualitätskriterien, anhand derer gemessen werden kann, wie gut ein Testfall ist. Dabei spielt es für die Auswertung keine Rolle, ob der Testfall manuell oder mit Hilfe einer LLM wie ChatGPT-4 von \textcite{OpenAI2025} erstellt wird. Anhand dieser hier dargestellten Ausschluss- und Qualitätskriterien werden die von ChatGPT-4 von \textcite{OpenAI2025} erstellten Testfälle, ausgewertet und nach ihrer Qualität gemessen. In dieser Arbeit gibt es zwei Arten von Metriken, zum einen die Ausschlusskriterien, Kriterien, welche die von ChatGPT-4 von \textcite{OpenAI2025} generierten Testfälle erfüllen müssen um bewertet zu werden und zum anderen die Qualitätskriterien, anhand derer diese mit manuellen Testfällen, die die gleichen Anforderungen abdecken, bewertet werden.
