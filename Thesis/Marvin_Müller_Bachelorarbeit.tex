...
\label{sec:abstract}
This bachelor thesis investigates the use of the LLM GPT-4o from \textcite{OpenAI2025} in the creation of requirement test cases, taking into account the requirement specifications provided to it. The aim of the work and the evaluation of the results is to find out whether GPT-4o is able to generate requirement test cases that are of high quality coverage, factual correctness and test case structure according to \textcite{IsoIecIeee2024}. A total of 128 test cases were generated for ten different test scenarios. To check factual correctness, these are checked against the information on the \textcite{OpenCart2025} website. Manually created test cases from \textcite{Pavankumar21github2022}, which test the same test scenarios, serve as a benchmark for coverage.\\
\\
...
Diese Bachelorarbeit erforscht den Einsatz des LLM GPT-4o von \textcite{OpenAI2025} bei der Erstellung von Anforderungstestfällen unter Berücksichtigung ihr zur Verfügung gestellten Anforderungsspezifikationen. Ziel der Arbeit und der Auswertung der Ergebnisse ist es herauszufinden, ob GPT-4o dazu in der Lage ist, Anforderungstestfälle zu erzeugen, welche in in Bezug auf ihre Abdeckung, Factual Correctness und Testfallstruktur nach \textcite{IsoIecIeee2024} eine hohe Qualität aufweisen. Dazu wurden für zehn unterschiedliche Testszenarien insgesamt 128 Testfälle generiert. Um die Factual Correctness zu überprüfen, werden diese mit den Informationen auf der \textcite{OpenCart2025} Webseite überprüft. Manuell erstellte Testfälle von \textcite{Pavankumar21github2022}, welche die gleichen Testszenarien testen, dienen als Benchmark zur Abdeckung.\\
\\
...
Kapitel \ref{sec:einleitung} widmet sich der Darstellung der Motivation, die der Bachelorarbeit zugrunde liegt und erläutert die Zielsetzung. Dabei wird verdeutlicht, warum das Thema Einsatz von Künstlicher Intelligenz
zur Testfallgenerierung relevant ist und welchen Beitrag die Arbeit dazu leistet. Darüber hinaus wird in diesem Kapitel die zentrale Forschungsfrage formuliert und die methodische Vorgehensweise in dieser Arbeit beschrieben. Diese Erläuterungen definieren den inhaltlichen und methodischen Rahmen dieser Arbeit.
...
\\
\textbf{Ist es möglich, dass das LLM GPT-4o von \textcite{OpenAI2025} dazu in der Lage ist, aus Anforderungsspezifikationen und Screenshots mit Website-Informationen, Anforderungstestfälle zu erzeugen, die eine hohe Qualität in Bezug auf ihre Abdeckung, Factual Correctness und Testfallstruktur nach \textcite{IsoIecIeee2024} aufweisen?}\\  
\\
Diese Frage zielt darauf ab, herauszufinden, wie gut ein frei verfügbares LLM wie GPT-4o in der Lage ist, Testfälle zu generieren und ob somit Nutzer wertvolle Zeit und Ressourcen sparen können, wenn die KI redundante Aufgaben wie das Erstellen von Anwendungstestfällen übernimmt. Die Ergebnisse müssen dann überprüft werden. Um diese Frage zu beantworten, werden Auswertungsmetriken entwickelt, anhand derer die von GPT-4o erstellten Testfälle ausgewertet werden können. Außerdem wird die gängige Testfallstruktur nach \textcite{IsoIecIeee2024} definiert und die faktische Korrektheit der Testfälle überprüft. Dabei dienen manuell erstellte Testfälle, welche aus der gleichen Anforderungsspezifikation erstellt wurden als Benchmark, also als Vergleichsmaßstab.
...
\\
\textcite{Hecker2018} nennt noch eine dritte Form der KI, die sogenannte Superintelligenz, diese wird es jedoch laut \textcite{Hecker2018} in nächster Zeit nicht geben und somit wird in dieser Arbeit nicht auf diese eingegangen. 
...
\\
RAG ist vielfältig einsetzbar und eignet sich in vielen Bereichen. Die Autoren \parencite{Gao2024, Lewis2021, Salemi2024} kommen zu dem Schluss, dass durch RAG die LLMs bessere Ergebnisse liefern können. Ergebnisse, welche RAG benutzen, zeichnen sich bei ihrer Performance aus und sind spezifischer sowie faktenreicher als Ergebnisse, die nur von dem LLM ohne RAG erzeugt wurden. (\cite{Gao2024}; \cite{Lewis2021}; \cite{Salemi2024})

\subsection{Einsatzmöglichkeiten von RAG} % fold
\label{sec:einsatzmöglichkeitenVonRAG}
RAG spielt in dieser Arbeit eine entscheidende Rolle. Mithilfe von RAG kann dem LLM eine Anforderungsspezifikation zur Verfügung gestellt werden, aus welcher bereits manuell Anforderungstestfälle generiert wurden. Durch RAG kann dem LLM diese Anforderungsspezifikation, welche eine PDF-Datei ist, verarbeiten und daraus Informationen ziehen. (\cite{Gao2024}; \cite{Wu2024})\\
\\
...
\\
Die Arbeit von \textcite{Kirinuki2024} vergleicht durch GPT-4 generierte Testfälle mit manuell erstellten Testfällen. Dabei handelt es sich um Black-Box-Tests. Die Autoren \textcite{Kirinuki2024} kommen zu dem Ergebnis, dass beim Aspekt der Testabdeckung die von GPT-4 erzeugten Testfälle gleich gut oder sogar besser abschneiden als von Menschen erstellte Testfälle. Diese Testabdeckung wird laut \textcite{Kirinuki2024} noch erhöht, wenn ein Mensch zusammen mit GPT-4 Testfälle erstellt. (\cite{Kirinuki2024})
\newpage

\noindent Diese Werke zeigen, dass KI und LLMs bereits in verschiedenen Testbereichen eingesetzt werden. Dabei handelt es sich um die Bereiche der Softwaretests, des Chip-Testing, der Unit Tests, des GUI-Testing und der Black-Box-Tests. Dies zeigt, dass es möglich ist, dass LLM wie GPT-4o in der Lage ist, Anwendungstestfälle zu generieren, welche eine bessere Qualität besitzen als manuell erstelle Anwendungstestfälle.
...
\\
Da GPT-4o Bilder erkennen und mit Dokumenten arbeiten kann, ist es in der Lage, die in Kapitel \ref{sec:rag} erklärte Methode RAG zu verwenden. Es kann somit Informationen aus den in Kapitel \ref{sec:testdaten} beschriebenen Anforderungsspezifikationen ziehen und dabei Webseiten und ihr Verhalten in Form von Screenshots erkennen und damit arbeiten. Außerdem besitzt GPT-4o eine Erinnerungsfunktion, mit welcher es auf wichtige Anmerkungen und Informationen aus vorherigen Prompts zurückgreifen kann.
\newpage

\noindent Es muss nicht mit viel Aufwand, Zeit und Ressourcen antrainiert oder ein eigenes Modell entwickelt werden. GPT-4o ist in vielen Sprachen nutzbar und kann so von Nutzern weltweit verwendet werden.\\
\\
...
\\
Dies wird überprüft, in dem untersucht wird, ob die Informationen aus den Testfällen von GPT-4o mit denen aus den manuellen Testfällen und auf der \textcite{OpenCart2025} Webseite übereinstimmen. Die Anzahl der Testfälle, die durch GPT-4o pro Testszenario erstellt wurden und dabei beide Qualitätskriterien erfüllen ist der Wert $TC_{GPT-4o}$.\\
\\
...
\\
Die Testdaten, werden von GPT-4o in eine Excel Datei eingetragen und dann nach den drei oben genannten Qualitätskriterien ausgewertet. In Kapitel \ref{sec:ergebnisse} finden sich die Ergebnisse. Eine Diskussion über die Ergebnisse und welche Schlüsse daraus gezogen werden können, findet sich in Kapitel \ref{sec:diskussion}.
\newpage
...
In Kapitel \ref{sec:ergebnisse} werden die Ergebnisse der von der GPT-4o von \textcite{OpenAI2025} erstellten Anforderungstestfälle präsentiert. Alle Testdaten, die Anforderungsspezifikationen, die manuellen sowie die von GPT-4 generierten Testfälle und die Auswertungstabellen finden sich im GitHub-Projekt von \textcite{Müller2025}.\\
\\
Im Folgenden werden die Ergebnisse, welche die von GPT-4o generierten Testfälle in Bezug auf die in Kapitel \ref{sec:qualitätskriterien} beschriebenen Qualitätskriterien \glqq Testfallstruktur\grqq{}, \glqq Factual Correctness\grqq{} und \glqq Abdeckung\grqq{} erreicht haben, veranschaulicht. In dieser Arbeit wurden zehn Testszenarien mit einer unterschiedlichen Anzahl an Testfällen getestet. Dabei wurde von GPT-4o insgesamt 128 Testfälle generiert.\\
\\
\textbf{\underline{1. Testfallstruktur}}\\
\\
Folgende Tabelle zeigt das Abschneiden der von GPT-4o erstellten Testfälle in Bezug auf das Qualitätskriterium \glqq Testfallstruktur\grqq{}, aufgelistet in den zehn Testszenarien und der Gesamtzahl:\\
\\
\textbf{Testfallstruktur:} 
\begin{table}[H]
 \begin{center}
...
\\
Folgende Tabelle zeigt das Abschneiden der von GPT-4o erstellten Testfälle in Bezug auf das Qualitätskriterium \glqq Factual Correctness\grqq{} in den zehn Testszenarien:\\
\\
\textbf{Factual Correctness:} 
\begin{table}[H]
 \begin{center}
...
\noindent Anhand dieser Grafik ist zu erkennen, dass die Factual Correctness pro Testszenario Schwankungen enthält, diese jedoch nie unter 50 \% fällt.
\newpage

\noindent\textbf{\underline{3. Coverage (Deutsch: Abdeckung)}}\\
\\
Folgende Tabelle zeigt das Abschneiden der von GPT-4o erstellten Testfälle in Bezug auf das Qualitätskriterium \glqq Abdeckung\grqq{} in den zehn Testszenarien: Dabei werden nur die Testfälle gezählt, welche sowohl im Qualitätskriterium \glqq Testfallstruktur\grqq{}, als auch im Qualitätskriterium \glqq Factual Correctness\grqq{} im positiven Bereich liegen:\\
\\
\textbf{Abdeckung:} 
\begin{table}[H]
 \begin{center}
...
\noindent Die von GPT-4o erstellten Testfälle werden, wie in Kapitel \ref{sec:coverage} beschrieben, mit der Anzahl der von \textcite{Pavankumar21github2022} erstellten Testfälle verglichen, um die Abdeckungsrate zu erhalten. Dabei gibt es in den zehn Testszenarien 147 von \textcite{Pavankumar21github2022} manuell erstellte Testfälle und 100 von GPT-4o generierte und als positiv eingestufte Testfälle. Dies ergibt eine Abdeckungsrate von 68 \%. Wie in dem Qualitätskriterium \glqq Factual Correctness\grqq{}, gibt es in dem Qualitätskriterium \glqq Abdeckung\grqq{} ebenfalls Schwankungen zwischen den einzelnen Testszenarien. Die niedrigste Abdeckungsrate mit einem Wert von 53,8 \% ist in dem Testszenario \glqq Change Password\grqq{} erzielt worden. Das Testszenario \glqq Currencies\grqq{} schneidet mit einer Abdeckungsrate von 100 \% am besten ab.\\
\newpage

\noindent Folgende Grafik zeigt anschaulich wie die Abdeckung pro Testszenario abschneidet:
...
\begin{figure}[H]
    ...
    \label{fig:enter-label}
\end{figure}
\noindent Anhand dieser Grafik ist zu erkennen, dass die Abdeckungsrate pro Testszenario Schwankungen enthält, diese jedoch nie unter 50 \% fällt.
% chapter ergebnisse (end)

\chapter{Diskussion} % (fold)
\label{sec:diskussion}
In dieser Arbeit wurde mit GPT-4o von \textcite{OpenAI2025} Anwendungstestfälle generiert. Dabei wurden diese anhand der in Kapitel \ref{sec:qualitätskriterien} beschriebenen Qualitätskriterien ausgewertet. In diesem Kapitel werden die Ergebnisse nach Qualitätskriterium eingeordnet.\\
\\
\textbf{\underline{1. Testfallstruktur:}}\\
\\
...
Diese vorliegende Arbeit untersuchte den Einsatz von Künstlicher Intelligenz zur Testfallgenerierung. Ziel dieser Arbeit ist es folgende Forschungsfrage zu beantworten:\\
\\
\textbf{Ist es möglich, dass das LLM GPT-4o von \textcite{OpenAI2025} dazu in der Lage ist, aus Anforderungsspezifikationen und Screenshots mit Website-Informationen, Anforderungstestfälle zu erzeugen, die eine hohe Qualität in Bezug auf ihre Abdeckung, Factual Correctness und Testfallstruktur nach \textcite{IsoIecIeee2024} aufweisen?}\\
\\
Um diese Frage zu beantworten, wurde durch GPT-4o Anwendungstestfälle generiert. Dabei wurden zehn unterschiedliche Testszenarien, mit insgesamt 128 Anwendungsfällen getestet. Diese Testfälle wurden dann ausgewertet.\\
\\
Aufgrund der vorliegenden Ergebnisse kommt diese Arbeit zu folgenden Schluss:\\
\noindent Es ist möglich, dass das LLM GPT-4o Anforderungstestfälle zu erzeugen, welche eine hohe Qualität in Bezug auf ihrer Abdeckung, Factual Correctness und Testfallstruktur aufweisen. Die Qualität ist jedoch von Testszenario zu Testszenario unterschiedlich, so dass keine allgemein gültige Aussage getroffen werden kann.\\
\\
...
\\
Die Factual Correctness erreichte den Durchschnittswert von 78,1 \%, wobei der niedrigste Wert 54,5 \% und der höchste Wert bei 100 \% liegt. Die restlichen acht Testszenarien lagen im Bereich zwischen 70 \% und 90 \%. Dies zeigt einen konstanten hohen Wert.\\
\\
Die von GPT-4o erstellten Testfälle erreichten bei der Abdeckungsrate einen Durchschnittswert von 68 \%. Der niedrigste Wert eines Testszenario liegt bei 53,8 \% und der höchste Wert bei 100 \%. Im Bereich 60 \% bis 88,9 \% liegen die restlichen acht Testszenarien.\\
\\
Die Ergebnisse dieser Arbeit zeigen, dass der Einsatz von Künstlicher Intelligenz zur Testfallgenerierung ein großes Potential besitzt. Zukünftige Werke können dazu andere LLMs verwenden und vergleichen und andere Anwendungen testen, um Optimierungsmöglichkeiten beim Einsatz von Künstlicher Intelligenz zur Testfallgenerierung zu untersuchen.
...
\\
Die Webseite \textcite{OpenCart2025} deren Funktionen von \textcite{Pavankumar21github2022} und von GPT-4o von \textcite{OpenAI2025} und getestet wurden: \url{https://www.opencart.com/}
